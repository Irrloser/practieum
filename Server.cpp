...
void Server::recvMessage(SOCKET socket){
	...
	if(receivedBytes == SOCKET_ERROR || receivedBytes == 0){//接受数据错误，把产生错误的会话socekt加入sessionsClosed队列
		...
		string Url;
		string fl;
		for (int i = 0; recvBuf[i] != '\n'; ++i)
			fl.push_back(recvBuf[i]);
		url2(recvBuf, Url);
		cout << fl << endl;
		string url2 = Config::url + Url;
		auto si = Url.end();
		si -= 3;
		int offset = si - Url.begin();
		//cout << Url.substr(offset,3)<<endl;
		//cout << url << endl;
		FILE* fp = fopen(url2.c_str(),"rb");
		//map<string, string> typemap;
		if (fp == NULL)
		{
			char* sBuf = new char[1024];
			char err[] = "HTTP/1.1 502 Bad Request \r\nServer: CG Server/0.1.0\r\n\r\n";
			fp = fopen("C:\\Users\\KatowiZz\\Desktop\\ChatRoomServerAsyn\\Debug\\error.htm", "rb");
			fseek(fp, 0, SEEK_END);
			int l = ftell(fp);
			fseek(fp, 0, SEEK_SET);
			char* erro = new char[l + 1];
			erro[l] = '\0';
			fread(erro, l, 1, fp);
			string tmp1(erro);
			tmp1 = err + tmp1;
			...
			fclose(fp);
			delete[] erro;
			cout << "open file error"<<(*clientAddrMaps)[socket] <<"port"<<Config::PORT<< endl;
			//sprintf()
			return;
		}
		fseek(fp, 0,SEEK_END);
		...
		char *buf = new char[len + 1000];
		if (Url.substr(offset, 3) == "jpg"|| Url.substr(offset, 3) == "png"||Url.substr(offset, 3) == "gif")
		{
			
			ifstream in(url2, ios::binary);
			if (in.is_open())
			{
				cout << Url.substr(offset, 3) << endl;
				istreambuf_iterator<char>beg(in), end;
				string str(beg, end);
				sprintf(buf, "HTTP / 1.1 200 OK\r\nConnection: Close\r\nContent-Length: %d\r\nContent-Type:image/%s\r\n\r\n",str.size(), Url.substr(offset, 3).c_str());
				str = buf + str;
				in.close();
				AddRecvMessage(str);
...
void Server::ForwardMessage(){
	//cout << numOfSocketSignaled << rcvedMessages->size() << endl;
	if(this->numOfSocketSignaled > 0){
		if(!this->rcvedMessages->empty()){//如果消息队列不为空
			//cout << "size:" << rcvedMessages->size() <<"  "<<numOfSocketSignaled<< endl;
			//auto sockItor = sessions->begin();
			for(list<string>::iterator msgItor = this->rcvedMessages->begin();msgItor != this->rcvedMessages->end();msgItor++){//对消息队列中的每一条消息
				for(list<SOCKET>::iterator sockItor = this->sessions->begin();sockItor != this->sessions->end();sockItor++){//对会话socket队列中的每个socket
					if( FD_ISSET(*sockItor,&this->wfds)){ 
						//cout << 1 << endl;
						cout <<"success  "<<(*clientAddrMaps)[*sockItor]<<"  port: "<< Config::PORT<< endl;
						this->sendMessage(*sockItor,*msgItor);
						sendMessage(*sockItor, "success");
					}
				}
			}
		}
...
void Server:: url2(char * recvbuf, string& s)
{
	int i = 0;
	while (recvbuf[i++] != '/');
	while(recvbuf[i] != ' ')
...
int Server::Loop(){
	...
	while(true){ //等待客户的连接请求
		...
		//首先检查是否有客户请求连接到来
		if(tmp=this->AcceptRequestionFromClient() != 0) return -1;	
		//然后向客户端发送数据
		this->ForwardMessage();
		//最后接受客户端发来的数据
